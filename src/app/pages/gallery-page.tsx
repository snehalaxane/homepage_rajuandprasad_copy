          ...
          <div className="container mx-auto px-6">
            ...
            >
              <div className="flex items-center justify-center gap-2 text-sm text-[#888888] mb-6">
                ...
                <span className="text-[#022683] font-semibold">Gallery</span>
              </div>
              ...
              <p className="text-lg lg:text-xl text-[#888888] leading-relaxed max-w-3xl mx-auto">
                {intro.subheading}
              </p>
            </motion.div>
          </div>
              ...
              {categories.map(category => (
                <button
                  ...
                  className={`px-6 py-3 rounded-full text-sm font-semibold transition-all cursor-pointer ${selectedCategory === category
                    ? 'bg-[#022683] text-white shadow-xl scale-105'
                    : 'bg-white text-[#888888] hover:bg-gray-100 shadow-md hover:shadow-lg border border-gray-200'
                    }`}
                >
                  {category}
                  {category !== 'All' && (
              ...
              {filteredImages.map((image, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-4">
                      {/* <h3 className="font-bold text-gray-900 mb-2 line-clamp-2 group-hover:text-[#022683] transition-colors">
                        {image.title || 'Untitled Image'}
                      </h3> */}
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="px-2 py-1 bg-[#022683]/5 text-[#022683] text-xs font-semibold rounded-full">
                          {image.category}
                        </span>
                      </div>
                    </div>
                    <div className="absolute inset-0 border-2 border-[#022683] opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl pointer-events-none" />
                  </motion.div>
                );
              })}
            </div>
            ...
            {filteredImages.length === 0 && !loading && (
              ...
              >
                <div className="w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-6">
                  <ImageIcon className="h-12 w-12 text-[#888888]" />
                </div>
                <h3 className="text-2xl font-bold text-gray-900 mb-3">No images found</h3>
                <p className="text-[#888888] mb-6">Try selecting a different category</p>
              </motion.div>
            )}
            ...
            >
              <div className="grid sm:grid-cols-2 gap-6 divide-y sm:divide-y-0 sm:divide-x divide-gray-200">
                <div className="text-center py-4 sm:py-0">
                  <div className="text-4xl font-bold text-[#022683] mb-2">{galleryImages.length}</div>
                  <div className="text-sm text-[#888888] font-semibold">Total Images</div>
                </div>
                <div className="text-center py-4 sm:py-0">
                  <div className="text-4xl font-bold text-[#022683] mb-2">{categories.length - 1}</div>
                  <div className="text-sm text-[#888888] font-semibold">Categories</div>
                </div>
                {/* <div className="text-center py-4 sm:py-0">
                  <div className="text-4xl font-bold text-[#022683] mb-2">
                    {Array.from(new Set(galleryImages.map(img => img.year))).length}
                  </div>
                  <div className="text-sm text-[#888888] font-semibold">Years of Memories</div>
                </div> */}
              </div>
            </motion.div>
          </div>
